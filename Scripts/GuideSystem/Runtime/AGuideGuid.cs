...
{
    [GuideDisplayType(typeof(AGuideGuid), "OnDrawGUI", "引导组件")]
    public class GuideGuidUtl
    {
        struct GuideInfo
        {
            public AGuideGuid guide;
            public string tag;
        }
        ...
        public static void OnAdd(AGuideGuid guid, bool bCheck = true)
        {
            ...
            for (int i =0; i < infoList.Count; )
            {
                ...
                if(guideInfo.guide.guid == guid.guid)
                {
                    ...
                    {
                        bFindTag = true;
                        var guideTag = AGuideTag.GetTag(guid);
                    ...
                    if(findTag == guideInfo.tag)
                    {
                        if (bGuideGuidLog)
                        {
                            Debug.LogErrorFormat("[AGuideGuid] {0}对应路径{1}下已存在相同PathTag的节点:{2}, 引导节点产生歧义, 请联系程序同学处理引导节点命名",
        ...
        {
            AGuideGuid guidWidget = FindGuide(value);
            ...
            AGuideGuid guide = (AGuideGuid)UnityEditor.EditorGUILayout.ObjectField(new GUIContent(displayName, "控件Id:" + value), guidWidget, typeof(AGuideGuid), true);
            GUI.color = color;
        ...
        //------------------------------------------------------
        public static int GeneratorGUID(AGuideGuid guide)
        {
        ...
        //------------------------------------------------------
        public static bool SetDirtyPrefab(AGuideGuid guide)
        {
            /*
            ...
            if (instance)
            {
                ...
                if (prefab != null)
                {
                    ...
                    if (childNode)
                    {
                        var temp = childNode.GetComponent<AGuideGuid>();
                        if (temp == null) temp = childNode.gameObject.AddComponent<AGuideGuid>();
                        temp.guid = guide.guid;

                        AEventTriggerListener eventTrigger = childNode.gameObject.GetComponent<AEventTriggerListener>();
                        if (eventTrigger == null) eventTrigger = childNode.gameObject.AddComponent<AEventTriggerListener>();
