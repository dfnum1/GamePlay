...
//    [CreateAssetMenu(menuName = "GamePlay/引导数据集")]
    public abstract class AGuideDatas : ScriptableObject
    {
        static AGuideDatas ms_Instacne = null;
        ...
        //------------------------------------------------------
        public static void Refresh(AGuideDatas aiObject = null)
        {
            if (aiObject == null)
            {
                string[] aiDatas = UnityEditor.AssetDatabase.FindAssets("t:AGuideDatas");
                if (aiDatas != null && aiDatas.Length > 0)
                {
                    aiObject = UnityEditor.AssetDatabase.LoadAssetAtPath<AGuideDatas>(UnityEditor.AssetDatabase.GUIDToAssetPath(aiDatas[0]));
                }
            }
    ...
    [UnityEditor.CustomEditor(typeof(AGuideDatas), true)]
    public class AGuideDataEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            serializedObject.Update();

            AGuideDatas controller = target as AGuideDatas;
            ...
            {
                AGuideDatas.Refresh(controller);
