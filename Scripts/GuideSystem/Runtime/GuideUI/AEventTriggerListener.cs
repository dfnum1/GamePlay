...
namespace Framework.Guide
{
    ...
    public class AEventTriggerListener : EventTrigger
    {
        ...
        //------------------------------------------------------
        public void SetGuideGuid(Framework.Guide.AGuideGuid guideGuid)
        {
            m_GuideGuid = guideGuid;
        }
        //------------------------------------------------------
        public void SetParentrate(AUIPenetrate parentrate)
        {
        ...
        //------------------------------------------------------
        public static AEventTriggerListener Get(GameObject go)
        {
            if (go == null) return null;
            AEventTriggerListener listener = go.GetComponent<AEventTriggerListener>();
            if (listener == null) listener = go.AddComponent<AEventTriggerListener>();
            return listener;
        }
        private void Awake()
        {
...
#if UNITY_EDITOR
    [CustomEditor(typeof(AEventTriggerListener))]
    [CanEditMultipleObjects]
    public class AEventTriggerListenerEditor : UnityEditor.Editor
    {
        private void OnEnable()
        {
            AEventTriggerListener evt = target as AEventTriggerListener;
        ...
        public override void OnInspectorGUI()
        {
            ...
            for (int i = 0; i < fields.Length; ++i)
            {
                ...
                if(fields[i].Name.CompareTo("m_GuideGuid") == 0)
                {
                    ...
                    if(guidValue == null)
                    {
                        fields[i].SetValue(evt, evt.gameObject.GetComponent<Framework.Guide.AGuideGuid>());
                    }
                }
            }
