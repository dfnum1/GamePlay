    ...
    public class GraphNode 
    {
        ...
        public void DrawPort(IPortNode portNode, GUIContent strLabel, System.Type displayType = null, string displayTypeLabel = null, bool bEdit = true, EBitGuiType bBit = EBitGuiType.None, EArgvFalg bPort = EArgvFalg.PortAll, float newLabelWidth = 84)
        {
            ...
            if (port is ArgvPort)
            {
                ...
                {
                    FieldInfo field = argv.GetType().GetField(strValueField, BindingFlags.NonPublic | BindingFlags.Instance);
                    if (field != null)
                    {
                ...
                {
                    FieldInfo field = argv.GetType().GetField(strIntField, BindingFlags.NonPublic | BindingFlags.Instance);
                    if (field != null)
                    {
