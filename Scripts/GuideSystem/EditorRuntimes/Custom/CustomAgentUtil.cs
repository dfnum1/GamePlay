        ...
        static GuideSystemEditor.NodeAttr BuildNodeAttr(AGuideCustomAgent.AgentUnit unit, string exprotName, bool bPreviewEditor = false)
        {
            ...
            if (unit.inputs != null && unit.inputs.Length > 0)
            {
                for (int a = 0; a < unit.inputs.Length; ++a)
                {
                    ...
                    newAttr.strTips = unit.inputs[a].tips;
                    newAttr.dispayTypeName = unit.inputs[a].displayType;
            ...
            if (unit.outputs != null && unit.outputs.Length > 0)
            {
                for (int a = 0; a < unit.outputs.Length; ++a)
                {
                    ...
                    newAttr.dispayTypeName = unit.outputs[a].displayType;
                    newAttr.strTips = unit.outputs[a].tips;
