...
namespace Framework.Core
{
    public struct RunPoint
    {
        ...
        public RunPoint(FVector3 pos, float speedScale =1)
        {
    ...
    public class Vector3Track
    {
        ...
        public void AddKeyPoint(FFloat fTime, FVector3 pos, bool bSort = true)
        {
            m_Keys.Add(new KeyPoint() { time = fTime, position = pos });
            if(bSort) SortUtility.QuickSortUp<KeyPoint>(ref m_Keys, 0);
        }
        //------------------------------------------------------
        public void InsertKeyPoint(int index, FFloat fTime, FVector3 pos)
        {
            if (index >= m_Keys.Count) AddKeyPoint(fTime, pos);
            else
            {
                if (index < 0) index = 0;
                ...
                m_Keys.Insert(index, new KeyPoint() { time = 0, position = pos });
            }
        }
        //------------------------------------------------------
        public void AddKeyPoint(FFloat fTime, RunPoint runPoint)
        {
            m_Keys.Add(new KeyPoint() { time = fTime, position = runPoint.position });
        ...
        //------------------------------------------------------
        public bool Evaluate(FFloat fTime, out FVector3 retPos)
        {
            retPos = FVector3.zero;
            FVector3 toPos;
            return Evaluate(fTime, out retPos, out toPos);
        }
        //------------------------------------------------------
        public bool Evaluate(FFloat fTime, out FVector3 retPos, out FVector3 toPos)
        {
            retPos = FVector3.zero;
            toPos = FVector3.zero;
    ...
    public class PosRotTrack
    {
        ...
        List<KeyPoint> m_Keys = new List<KeyPoint>();
        public void AddKeyPoint(FFloat fTime, FVector3 pos, FVector3 rot, bool bSort = true)
        {
            m_Keys.Add(new KeyPoint() { time = fTime, position = pos, rotation = rot });
            if (bSort) SortUtility.QuickSortUp<KeyPoint>(ref m_Keys, 0);
        }
        //------------------------------------------------------
        public void InsertKeyPoint(int index, FFloat fTime, FVector3 pos, FVector3 rot)
        {
            if (index >= m_Keys.Count) AddKeyPoint(fTime, pos, rot);
            else
            {
                if (index < 0) index = 0;
                for (int i = 0; i < m_Keys.Count; ++i)
                ...
                m_Keys.Insert(index, new KeyPoint() { time = 0, position = pos, rotation = rot });
            }
        }
        //------------------------------------------------------
        public void AddKeyPoint(FFloat fTime, RunPoint runPoint)
        {
            m_Keys.Add(new KeyPoint() { time = fTime, position = runPoint.position, rotation = runPoint.rotation});
            SortUtility.QuickSortUp<KeyPoint>(ref m_Keys, 0);
        }
        //------------------------------------------------------
        public void DelKeyPoint(FFloat fTime)
        ...
        //------------------------------------------------------
        public bool Evaluate(FFloat fTime, out FVector3 retPos, out FVector3 retRotation)
        {
            retPos = FVector3.zero;
