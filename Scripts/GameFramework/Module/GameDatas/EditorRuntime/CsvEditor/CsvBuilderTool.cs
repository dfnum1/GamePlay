...
namespace Framework.ED
{
    public class CsvBuilderTool
    {
        ...
        {
            if(string.IsNullOrEmpty(ms_strEditorPath))
            {
                ms_strEditorPath = EditorUtils.GetInstallEditorResourcePath();
                if (!string.IsNullOrEmpty(ms_strEditorPath))
                {
                    ms_strEditorPath = Path.Combine(ms_strEditorPath, "GameData");
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(EditorFrameworkPreferences.GetSettings().tableCsServerGeneratedPatch))
            {
                UnityEngine.Debug.LogWarning("请先设置配置表c#服务器代码生成路径");
                return;
            }
            HashSet<string> vServerTables = ParseServerTableConfig();
            if (bBuildCode) BuildCode(vServerTables, true, EditorFrameworkPreferences.GetSettings().tableCsServerGeneratedPatch);
            ...
            //             //! copy binary data
            //             {
            //                 string strCreateFileMapping = Application.dataPath + EditorFrameworkPreferences.GetSettings().tableCsServerGeneratedPatch + "Config/Binarys/";
        ...
        //------------------------------------------------------
        public static void BuildCode(HashSet<string> vContainTables = null, bool bServerCode = false, string rootDir = "")
        {
                    ...
                    if (bBuildOk)
                    {
                        ...
                        if (Directory.Exists(srcDir))
                        {
                            ...
                            if (bServerCode)
                            {
                                ...
                                if (Directory.Exists(destSvrDir))
                                {
                                    EditorUtils.ClearDirectory(destSvrDir);
                                    for (int i = 0; i < csvs.Length; ++i)
                                    {
                            ...
                            else
                            {
                                //! copy to unity project
                                {
                                    ...
                                    EditorUtils.ClearDirectory(destDir);
                                    for (int i = 0; i < csvs.Length; ++i)
                                    {
                                ...
                                // build cpp
                                if(!string.IsNullOrEmpty(EditorFrameworkPreferences.GetSettings().tableCppGeneratedPatch))
                                {
                                    bool HasCppGenerator = false;
                                    string destDir = Application.dataPath + EditorFrameworkPreferences.GetSettings().tableCppGeneratedPatch;
                                    if (Directory.Exists(destDir))
                                    ...
                                    {
                                        string destFile = Path.Combine(destDir, Path.GetFileName(tmp));
        ...
        {
            string strRoot = Path.Combine(Application.dataPath, EditorFrameworkPreferences.GetSettings().tableCppGeneratedPatch + "../").Replace("\\","/");
