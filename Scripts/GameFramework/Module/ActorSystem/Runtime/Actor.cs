    ...
    public class Actor : TypeActor, ICutsceneObject
    {
        ...
        public virtual Matrix4x4 GetEventBindSlot(string strSlot, int bindSlot = (int)ESlotBindBit.All)
        {
            ...
            if (strSlot.Equals("RootTop", StringComparison.OrdinalIgnoreCase))
            {
                ...
                BaseUtil.OffsetPosition(ref temp, new FVector3(0, m_pActorParameter.GetModelHeight(), 0));
                return temp;
            }
            ...
            if (slotTransform)
            {
                if ((bindSlot & (int)ESlotBindBit.Rotation) != 0)
                {
                    ...
                    if ((bindSlot & (int)ESlotBindBit.Position) == 0) BaseUtil.UpdatePosition(ref matrix, GetPosition()+ slotOffset);
                    if ((bindSlot & (int)ESlotBindBit.Scale) == 0) BaseUtil.UpdateScale(ref matrix, GetScale());
                }
                else
                {
                    if ((bindSlot & (int)ESlotBindBit.Position) != 0) BaseUtil.UpdatePosition(ref matrix, slotTransform.position + slotOffset);
                    if ((bindSlot & (int)ESlotBindBit.Scale) != 0) BaseUtil.UpdateScale(ref matrix, slotTransform.localScale);
                }
            }
