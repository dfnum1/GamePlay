...
{
    public class GameModeTypeProvider : ScriptableObject, ISearchWindowProvider
    {
        ...
        {
            var provider = ScriptableObject.CreateInstance<Editor.GameModeTypeProvider>();
        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            tree.Add(new SearchTreeGroupEntry(new GUIContent("选择游戏模式")));
            var nodes = StateEditorUtil.GetStateWorldTypes();
            if (nodes != null)
            {
                foreach (var node in nodes)
                {
                    if (!node.Value.IsSubclassOf(typeof(AMode)))
                        continue;
