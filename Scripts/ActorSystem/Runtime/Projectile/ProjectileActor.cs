...
namespace Framework.ActorSystem.Runtime
{
    ...
    {
        private ProjectileManager m_pProjectileSystem;
  ...
  //      private FVector3 m_Position;
   ...
   //     private FVector3 m_Direction;
        ...
        private AActorStateInfo m_pStateParam;
        ...
        //------------------------------------------------------
        public void SetStateParam(AActorStateInfo value) => m_pStateParam = value;
        //------------------------------------------------------
        ...
        //------------------------------------------------------
        public Actor GetBoundStartActor() => m_pBoundStartActor;
        //------------------------------------------------------
        public void SetBoundStartActor(Actor value) => m_pBoundStartActor = value;
        //------------------------------------------------------
        public int GetRemainBoundCount() => m_nRemainBoundCount;
        //------------------------------------------------------
        public void SetRemainBoundCount(int value) => m_nRemainBoundCount = value;
        //------------------------------------------------------
        public Actor GetOwnerActor() => m_pOwnerActor;
        //------------------------------------------------------
        public void SetOwnerActor(Actor value) => m_pOwnerActor = value;
        //------------------------------------------------------
        ...
        //------------------------------------------------------
        public Actor GetTargetNode() => m_pTargetNode;
        //------------------------------------------------------
        public void SetTargetNode(Actor value) => m_pTargetNode = value;
        //------------------------------------------------------
    ...
    //        m_Position = FVector3.zero;
     ...
     //       m_Direction = FVector3.zero;
        ...
        //------------------------------------------------------
        public void SetData(ProjectileData pData, Actor pOwnerActor, Actor pTargetNode, FVector3 vPosition, FVector3 vDirection, int nDamagePower, uint nTrackFrameId, uint track_body_id)
        {
            ...
            if (m_Speed.sqrMagnitude <= 0.001f)
                ...
                dir = m_Speed;
            m_SelfRotate.x = GetActorManager().GetRandom(pData.minRotate.x, pData.maxRotate.x);
            m_SelfRotate.y = GetActorManager().GetRandom(pData.minRotate.y, pData.maxRotate.y);
            m_SelfRotate.z = GetActorManager().GetRandom(pData.minRotate.z, pData.maxRotate.z);
            dir.Normalize();
            SetDirection(dir);
        ...
        public void SetSpeed(FVector3 vPosition, FVector3 vDirection, int index, bool isFace2D = false)
        {
            ...
            {
                vDirection.Normalize();
                this.m_Speed.x = GetActorManager().GetRandom(m_ProjecileData.speedLerp.x, m_ProjecileData.speedLerp.y);
            ...
            if (index >= 0 && m_ProjecileData.speedLows != null && index < m_ProjecileData.speedLows.Length)
            {
                ...
                {
                    randomSpeed.x = GetActorManager().GetRandom(m_ProjecileData.speedLows[index].x, m_ProjecileData.speedUppers[index].x);
                    randomSpeed.y = GetActorManager().GetRandom(m_ProjecileData.speedLows[index].y, m_ProjecileData.speedUppers[index].y);
                    randomSpeed.z = GetActorManager().GetRandom(m_ProjecileData.speedLows[index].z, m_ProjecileData.speedUppers[index].z);
                }
        ...
        public void SetBoundSpeed(FVector3 vPosition, FVector3 vDirection, int index)
        {
            ...
            {
                vDirection.Normalize();
                m_Speed = vDirection * GetActorManager().GetRandom(m_ProjecileData.speedLerp.x, m_ProjecileData.speedLerp.y);
                m_HasSpeed = m_Speed.sqrMagnitude > 0.1f;
                return;
            }
        ...
        protected override void OnUpdate(float fFrameTime)
        {
            ...
            switch (m_ProjecileData.bornType)
            {
                case EProjecitleBornType.FollowTrigger:
                    {
                        if (m_pOwnerActor == null || m_pOwnerActor.IsFlag(EActorFlag.Killed) || m_pOwnerActor.IsDestroy())
                        {
                            m_fRemainLifeTime = 0;
                        }
                        else
                ...
                default:
                    {
                        ...
                        else if (m_ProjecileData.type == EProjectileType.Bounce)
                        {
                            if (m_fDelayTime <= 0)
                            {
                                m_Speed.y -= fFrameTime * ActorSystemUtil.GTRAVITY_VALUE;
                                if (IsTerrainBelow())
                                {
                                    if (m_Speed.y < 0.0f)
                                    {
                        ...
                        if (m_fDelayTime > 0)
                        {
                            if (m_pOwnerActor != null)
                            {
                                ...
                                {
                                    var bindSlot = this.m_pOwnerActor.GetEventBindSlot(this.m_strBindOwnerSlot);
                                    if (bindSlot != null)
                                    {
                                        FVector3 vPosition = bindSlot.GetPosition();
                                        if (this.m_StartBindOffset.sqrMagnitude > 0)
                                        {
                                            FQuaternion rotation = bindSlot.rotation;
                                            Vector3 forward = rotation*Vector3.forward;
                                            Vector3 right = rotation * Vector3.right;
                                            Vector3 up = rotation * Vector3.up;
                                            vPosition += forward * this.m_StartBindOffset.z;
                                            vPosition += right * this.m_StartBindOffset.x;
                                            vPosition += up * this.m_StartBindOffset.y;
                                        }
                        ...
                        if ((m_ProjecileData.bound_flag & (int)EBoundFlag.PhysicReflectBound) != 0 && m_nRemainBoundCount > 0)
                        {
                            //! check bound
                            RaycastHit hit;
                            if (curSpeed.sqrMagnitude > 0 && Physics.Raycast(GetPosition(), GetDirection(), out hit, curSpeed.magnitude * fFrameTime, m_pSytstem.GetTerrainLayerMask()))
                            {
        ...
        public bool IsTerrainBelow()
        {
            ...
            int terrainLayerMask = m_pSytstem != null ? m_pSytstem.GetTerrainLayerMask() : -1;
            float checkDistance = 5.0f;
            RaycastHit hit;
            if (Physics.Raycast(position, Vector3.down, out hit, checkDistance, terrainLayerMask))
            {
        ...
        //------------------------------------------------------
        internal bool CheckIntersection(ProjectileManager pMgr,out bool isCounteract, out bool isRangeExplode, out bool isHitScene)
        {
            isCounteract = false;
            ...
            var cacheHits = m_pSytstem.GetHitFrameActorCaches();
            var cacheWorldNodes = m_pSytstem.GetCatchActorList();
            ...
            if (ProjectileData.IsTrack(m_ProjecileData.type))
            {
                        ...
                        && !IsHited(m_pTargetNode.GetInstanceID(), false))
                {
                    ...
                    if (bTrackCheck)
                    {
                        ...
                        if (bFrameHit)
                        {
                            AActorStateInfo state_param = m_pTargetNode.GetStateParam();
                            ...
                            if (hit.hitType == EHitType.Bound)
                                hit.hitType_take_data_0 = m_ProjecileData.bound_count - m_nRemainBoundCount;
                            else if (IsPenetrable())
                            {
                                hit.hitType = EHitType.MutiHit;
                                hit.hitType_take_data_0 = m_ProjecileData.hit_count - m_nRemainHitCount;
                            }
                ...
                for (int c = 0; c < neighborActors; ++c)
                {
                    ...
                    if (bCheck)
                    {
                        if (m_nTrackBodyPart != 0xffffffff)
                        {
                            ...
                            if (actorTarget != null)
                                CU_ProjectileAttackFrameInsector(this, actorTarget, actorTarget.GetCurrentPlayActionState(), ref cacheHits, ref bPenetrableHit);
                        }
            ...
            else if (CanSceneTest())
            {
                ...
                if (Physics.Raycast(GetPosition(), GetUp(), out hit, 0.1f, m_pSytstem.GetTerrainLayerMask()))
                {
                    hitPos = hit.point;
                    SetPosition(hitPos);
                    isHitScene = true;
                ...
                if(pAttacker != null)
                {
                    foreach (var hitData in cacheHits)
                    {
                        if (hitData.target_ptr != null)
                        {
                            ...
                            {
                                m_pSytstem.OnActorSystemActorHitFrame(hitData);
                            }
                        }
                    }
        ...
        //------------------------------------------------------
        public bool TestFinalDropPos(float deltaTime)
        {
            m_HasDropPoint = false;
            m_FinalDropPosition = Vector3.zero;
            ...
            float fDelta = Mathf.Max(deltaTime, m_ProjecileData.life_time / 10);
        ...
        //------------------------------------------------------
        public FVector3 GetFrameHitPosition(Actor pTarget = null)
        {
            if (pTarget == null) pTarget = m_pTargetNode;
            if (pTarget != null)
            {
                var eventSlot = pTarget.GetEventBindSlot(m_ProjecileData.effect_hit_slot);
                if (!string.IsNullOrEmpty(m_ProjecileData.effect_hit_slot))
                {
                    var rotation = eventSlot.rotation;
                    Vector3 forward = rotation * Vector3.forward;
                    Vector3 right = rotation * Vector3.right;
                    Vector3 up = rotation * Vector3.up;
                    return eventSlot.GetPosition() + forward * m_ProjecileData.target_effect_hit_offset.z + right * m_ProjecileData.target_effect_hit_offset.x + up * m_ProjecileData.target_effect_hit_offset.y;
                }
        ...
        //------------------------------------------------------
        public FVector3 GetFrameExplodePosition(Actor pTarget = null)
        {
            if (pTarget != null)
            {
        ...
        public void RecodeHit(int nFlagID, float fDelay = 0)
        {
            ...
            m_on_hit_actors[nFlagID] = (int)(m_pSytstem.GetRunTime() + (int)(fDelay * 1000));
        }
        //------------------------------------------------------
        public override bool IsIntersecition(Actor pNode)
        {
            ...
            {
                if (IntersetionUtil.CU_SphereAABBInstersection(GetPosition(), (FFloat)m_ProjecileData.aabb_min.x, pNode.GetPosition(), FVector3.one * 0.01f))
                    return true;
            }
            return base.IsIntersecition(pNode);
        }
        //------------------------------------------------------
        public override bool IsIntersecition(FMatrix4x4 mtTrans, FVector3 vCenter, FVector3 vHalf)
        {
            ...
            if (m_ProjecileData.collisionType == EProjectileCollisionType.CAPSULE)
            {
                ...
                if (IntersetionUtil.CU_SphereAABBInstersection(GetPosition(), (FFloat)m_ProjecileData.aabb_min.x, vTransCenter - vHalf, vTransCenter+vHalf))
                    return true;
            }
            return base.IsIntersecition(mtTrans, vCenter, vHalf);
        }
        //------------------------------------------------------
        public override bool IsIntersecition(FMatrix4x4 mtTrans, FFloat radius)
        {
            ...
            if (m_ProjecileData.collisionType == EProjectileCollisionType.CAPSULE)
            {
                ...
                if (IntersetionUtil.CU_SphereSphereInstersection(GetPosition(), (FFloat)m_ProjecileData.aabb_min.x, vTransCenter, radius))
                    return true;
            }
            return base.IsIntersecition(mtTrans,radius);
        }
        //------------------------------------------------------
        public bool IsHited(int nFlag, bool bCheckTime = true)
        {
            ...
            if (m_on_hit_actors.TryGetValue(nFlag, out lastTime))
            {
                ...
                {
                    if (bCheckTime)
                        return (m_pSytstem.GetRunTime() - lastTime) < GetHitStep();
                    return true;
                }
            }
            return false;
        ...
        //------------------------------------------------------
        public void CopyHits(ProjectileActor pProj)
        {
        ...
        //------------------------------------------------------
        public bool IsBounded(Actor pNode)
        {
            if (m_bounded_list == null) return false;
            int id = pNode.GetInstanceID();
            return m_bounded_list.Contains(id);
        }
        //------------------------------------------------------
        public void CopyBoundeds(ProjectileActor pProj)
        {
        ...
        //------------------------------------------------------
        public void AddBounded(Actor pNode)
        {
            if (m_bounded_list == null) m_bounded_list = new HashSet<int>(2);
            int id = pNode.GetInstanceID();
        ...
        //------------------------------------------------------
        public bool CanHit(Actor pActor)
        {
            ...
            {
                if (m_on_hit_actors != null && m_on_hit_actors.ContainsKey(pActor.GetInstanceID()))
                    return false;
            }
            return true;
        }
        ...
        public void BuildTrackPathKeyframe(FVector3 vStart, FVector3 vEnd, float fSpeed = 0)
        {
            ...
            {
                if (fSpeed == 0)
                    this.m_Speed.x = m_pSytstem.GetRandom(m_ProjecileData.speedLerp.x, m_ProjecileData.speedLerp.y);
                else
...
#if USE_FIXEDMATH
                            ProjectileKeyframe keyFrame = new ProjectileKeyframe(0, m_ProjecileData.accelerations[0].x, ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedMaxs[0], vRot) + vStart, FVector3.zero, ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedUppers[0], vRot));
                            m_vTrackKeyframes.Add(keyFrame);

                            keyFrame = new ProjectileKeyframe(fPathLength / m_Speed.x, m_ProjecileData.accelerations[1].x, ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedMaxs[1], vRot) + vEnd, ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedLows[1], vRot), FVector3.zero);
                            m_vTrackKeyframes.Add(keyFrame);
#else
                            ProjectileKeyframe keyFrame = new ProjectileKeyframe(0, m_ProjecileData.accelerations[0].x, ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedMaxs[0], vRot) + vStart, FVector3.zero, ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedUppers[0], vRot));
                            m_vTrackKeyframes.Add(keyFrame);

                            keyFrame = new ProjectileKeyframe(fPathLength / m_Speed.x, m_ProjecileData.accelerations[1].x, ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedMaxs[1], vRot) + vEnd, ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedLows[1], vRot), FVector3.zero);
                            m_vTrackKeyframes.Add(keyFrame);
#endif
...
#if USE_FIXEDMATH
                            FVector3 pt = ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedMaxs[i], vRot);
#else
                            FVector3 pt = ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedMaxs[i], vRot);
#endif
...
#if USE_FIXEDMATH
                            FVector3 inTan = ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedLows[i], vRot);
                            FVector3 outTan = ActorSystemUtil.FRoateAround(FVector3.zero, m_ProjecileData.speedUppers[i], vRot);
#else
                            FVector3 inTan = ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedLows[i], vRot);
                            FVector3 outTan = ActorSystemUtil.RoateAround(FVector3.zero, m_ProjecileData.speedUppers[i], vRot);
#endif
        ...
        //------------------------------------------------------
        public static void CU_ProjectileAttackFrameInsector(ProjectileActor pProjectile, Actor pTarget, ActorAction targetAction, ref HashSet<HitFrameActor> vHitFrames, ref bool bPenetrableHit)
        {
            Vector3 vMin1, vMax1;
            Matrix4x4 targetWorld = pTarget.GetMatrix();
            ...
            if (pProjectile.IsIntersecition(targetWorld, vMin1, vMax1))
            {
                HitFrameActor hit = new HitFrameActor(pProjectile.GetDamageID(), pProjectile, pTarget, pProjectile.GetProjectileData(), pProjectile.GetFrameHitPosition(pTarget), pProjectile.GetDirection(),
                    0, pProjectile.GetStateParam());
                hit.damage_power += pProjectile.GetDamagePower();
                hit.hitType = pProjectile.IsBoundProjectile() ? EHitType.Bound : EHitType.Unknown;
                if (hit.hitType == EHitType.Bound)
                    hit.hitType_take_data_0 = pProjectile.GetCfgBoundCount() - pProjectile.GetRemainBoundCount();
                else if (pProjectile.IsPenetrable())
                {
                    hit.hitType = EHitType.MutiHit;
                    hit.hitType_take_data_1 = pProjectile.GetCfgHitCount() - pProjectile.GetRemainHitCount();
                }
        ...
        {
            UnityEditor.Handles.Label(GetPosition(), this.GetType().Name + ":" + GetInstanceID().ToString());
            if (m_ProjecileData == null) return;
            ...
            {
                ActorSystemUtil.DrawBoundingBox(GetPosition(), Vector3.one * m_ProjecileData.explode_range, Matrix4x4.identity, Color.cyan, true);
            }
